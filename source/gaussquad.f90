module gaussquad
  ...
  character(len=*), parameter :: VERSION = "2.5"
  ...
  intrinsic :: abs, epsilon, sqrt, sign, log_gamma
  ...
  subroutine recur_coeffs(icode, n, alpha, beta, a, b, muzero, info)
    ...
    case(JACOBI)
      ...
      muzero = TWO**(ab + ONE) * exp( &
               log_gamma(alpha+ONE) + log_gamma(beta+ONE) - log_gamma(abi) )
      a(1) = (beta - alpha)/abi
      ...
      return
    case(LAGUERRE)
      muzero = exp( log_gamma(alpha+ONE) )
      do i = 1, n
        a(i) = TWO*i - ONE + alpha
        b(i) = sqrt(i*(i+alpha))
      end do
